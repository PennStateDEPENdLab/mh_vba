function [p_sub,o_sub,p_group,o_group] = VBA_MFX_parallel(y,u,f_fname,g_fname,dim,options,priors_group, options_group)
...
if ~exist('priors_group', 'var'), priors_group=[]; end
...
while ~stop
  fprintf("On VB MFX iteration %d\n", it);
    ...
    parfor i=1:ns
        ...
        [p_sub{i}, otemp] = VBA_NLStateSpaceModel(y{i},u{i},f_fname,g_fname,dim,options{i},in);
        otemp.options.init = []; %Clear large unused objects from memory
        o_sub{i} = otemp; %populate group structure
