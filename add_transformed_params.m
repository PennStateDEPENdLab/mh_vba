...
posterior.transformed.SigmaPhi = transform_phi(posterior.SigmaPhi, vo, true);

posterior.transformed.muTheta = transform_theta(posterior.muTheta, vo);
posterior.transformed.SigmaTheta = transform_theta(posterior.SigmaTheta, vo, true);
...
if isfield(posterior, 'ffx')
  ...
  posterior.transformed.SigmaPhi_ffx = transform_phi(posterior.ffx.SigmaPhi, vo, true);
  
  posterior.transformed.muTheta_ffx = transform_theta(posterior.ffx.muTheta, vo);
  posterior.transformed.SigmaTheta_ffx = transform_theta(posterior.ffx.SigmaTheta, vo, true);
end
